...
void* reverse_sentence(void* arg) {
    int thread_id = *(int*)arg;
    char temp[50];

    pthread_mutex_lock(&mutex);

    // Reverse pairs of words in the sentence
    int start = thread_id * (word_count / NUM_THREADS) * 2;
    int end = start + (word_count / NUM_THREADS) * 2 - 1;
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        thread_ids[i] = i;
        pthread_create(&threads[i], NULL, reverse_sentence, &thread_ids[i]);
    }
